...
To refresh an `access_token`, the `client_secret` is required. This is a static, confidential security credential already known at build time. It must be protected from unauthorized access. Only the app should be able to read it.

TODO No solution approach yet. Consider https://developer.okta.com/blog/2019/01/22/oauth-api-keys-arent-safe-in-mobile-apps.

TODO OAuth only requires this parameter if it was issued (if I understand https://datatracker.ietf.org/doc/html/rfc6749#section-6 correctly). So: Could the Hub team create an API client for us, that has not client_secret?
